...
public class PwdHandler {
	...
	private static final int ITERATIONS = 10000; // nb de fois que l'algo va s'executer sur le mdp, genre hashN =
													// h(hashN-1), hashN-1 = h(hashN-2)
	...
	// Comme ça, meme si deux personnes ont comme mdp "abcd", ils n'auront pas le
	// meme hash car ils auront deux salt différent
	// Plus sécurisé et pas beaucoup + difficile, ça protège des attaques par
	// dictionnaire.
	...
	public Optional<String> hashPwd(String pwd, String salt) {
		...
		PBEKeySpec spec = new PBEKeySpec(chars, bytes, ITERATIONS, KEY_LENGTH); // on créer les specifications du hash
																				// qu'on veut générer

		try {
			SecretKeyFactory factory = SecretKeyFactory.getInstance(ALGORITHM);
			byte[] hashedPwd = factory.generateSecret(spec).getEncoded(); // et pour génerer le hash on se sert bien de
																			// ses specs
